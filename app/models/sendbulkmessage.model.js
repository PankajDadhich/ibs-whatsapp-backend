...
async function sendBulkMessage(tenants) {
  try {
    ...
    if (uniqueArray.length > 0) {
        console.log(
            "campaignData.whatsapp_number_admin->",campaignData.whatsapp_number_admin
        );
        
        if(campaignData.whatsapp_number_admin){
            //campaignData.Number=campaignData.whatsapp_number_admin
          
            uniqueArray.push(  { Name: 'pankaj dadhich', Number: campaignData.whatsapp_number_admin })
        }
  ...
  try {
    ...
    if (campaignData.header !== 'TEXT' && campaignData.header && campaignData.header_body) {
      if (campaignData.params_file_id) {
        console.log("campaignData.params_file_id->", campaignData.params_file_id);
        ...
        //const filePath =`https://sandbox.watconnect.com/public/demo/attachment/2103975140019816.jpeg`;
      ...
      } else if (campaignData.header === 'IMAGE' || campaignData.header === 'VIDEO' || campaignData.header === 'DOCUMENT') {
        documentId = await uploadWhatsAppMedia(campaignData.header, campaignData.header_body, tenants, campaignData.business_number);
      }
    }
    ...
    for (let record of data) {
      if (stopSending) {
            ...
            console.log(`Skipping message sending for ${record.Number} because of previous failure.`);
        ...
        result = await sendMessage(payload, record, campaign_id, file_id, createdbyid, template_name, message_template_id, tenants, business_number);

      if (result.status === 'Failed') {
        console.log(`Message to ${record.Number} failed, stopping further sends.`);
        stopSending = true;  
      }
    }
//
...
async function createMessagePayload(record, campaignData, tenants) {
  const parsedBodyTextParams = JSON.parse(campaignData.body_text_params);
...
async function sendMessage(payload, record, campaign_id, file_id, createdbyid, template_name, message_template_id, tenants, business_number) {
    try {
      ...
      if (response.ok) {
        // Ensure messages is defined and contains an element before accessing it
