...
//     let targetSchema = newCompanyWithUser["schema"].target_schemaname;
...
//         `INSERT INTO public.user (firstname, lastname, password, email, whatsapp_number, userrole, companyid)  VALUES ($1, $2, $3, $4, $5, $6, $7) RETURNING *`,
//         [
//           userInfo.firstname,
//           userInfo.lastname,
//           cryptPassword,
...
async function createCompanyWithUser(newCompanyWithUser, cryptPassword) {
  ...
  try {
    ...
    // Insert company details into the public schema
    const result = await sql.query(
    ...
    if (result.rows.length > 0) {
      ...
      // Invoice and subscription logic
      let start_date = moment(invoice.date);
      let end_date, due_date, validity = 5, invoiceStatus;
      ...
      // Clone schema
      try {
        await pgSchema.cloneSchemaWithoutData(
          sourceschema,
          targetSchema
        );
console.log("targetSchema",targetSchema);
        // Insert user into the cloned schema
        let userInfo = newCompanyWithUser["user_info"];
        const userResult = await sql.query(
          `INSERT INTO ${targetSchema}.user (firstname, lastname, password, email,  whatsapp_number, userrole, companyid)  
...
console.log("userResult",userResult);
        await sql.query("COMMIT");
        return { id: userResult.rows[0].id, company_id: company_id, invoice_id: invoiceResult.rows[0].id };
      } catch (error) {
        await sql.query("ROLLBACK");
...
//       const userResult = await sql.query(
//         `UPDATE public.user SET firstname=$2, lastname=$3, email=$4, whatsapp_number=$5, companyid=$6, isactive=$7 WHERE id=$1`,
//         [
//           userInfo.userId,
//           userInfo.firstname,
//           userInfo.lastname,
...
async function updateCompanyWithUser(newCompanyWithUser) {
  ...
  try {
    ...
    if (result.rowCount > 0) {
      ...
      let invoice = newCompanyWithUser["invoice"];
      let start_date = moment(invoice.date);
      let end_date, due_date, validity = 5;
      //  invoiceStatus;
      ...
      try {
        await pgSchema.updateSchemaWithoutData(
          sourceschema,
          targetSchema
        );
        ...
        const userResult = await sql.query(
          `UPDATE ${targetSchema}.user 
           SET firstname=$2, lastname=$3, email=$4, whatsapp_number=$5, companyid=$6, isactive=$7 
           WHERE id=$1`,
          [
            userInfo.userId,
            userInfo.firstname,
